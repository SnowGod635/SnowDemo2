...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'snow': // Week Snow
				...
				add(bg);
			case 'snowNew': // Week Snow Updated
				/*var bg:BGSprite = new BGSprite('castle/fundo', -400, -200);
				bg.scale.set(0.7, 0.7);
				add(bg);

				var porta:BGSprite = new BGSprite('castle/porta', 50, 100);
				porta.scale.set(1.2, 1.2);
				add(porta);

				var pilares:BGSprite = new BGSprite('castle/pilares', 50, 100);
				pilares.scale.set(1.2, 1.2);
				add(pilares);

				var teto:BGSprite = new BGSprite('castle/teto', 50, 100);
				teto.scale.set(1.2, 1.2);
				add(teto);

				var statuas:BGSprite = new BGSprite('castle/statuas', 50, 100);
				statuas.scale.set(1.2, 1.2);
				add(statuas);*/

				var bg:BGSprite = new BGSprite('castleNewNew', -400, -300);
		...
		if(curSong == 'ice-cap')
		{
			tbc = new FlxText(0, 0, 0, 'TO BE CONTINUED...', 40);
			tbc.color = FlxColor.BLACK;
			tbc.cameras = [camHUD];
			tbc.screenCenter();
			tbc.alpha = 0;
			add(tbc);
  ...
  public function clearShaderFromCamera(cam:String){
	...
	public function startVideo(name:String):Void {
		...
		if(foundFile) {
			...
			(new FlxVideo(fileName)).finishCallback = function() {
				...
				{
					endSong();
				} else if (curSong.toLowerCase() == 'snowfall')
				{
					startDialogue(dialogueJson);
				}
				else {
	...
	override function stepHit()
	{
		...
		if (curStep == 1936 && curSong.toLowerCase() == 'ice-cap')
		{
			new FlxTimer().start(0.5, function(tmr:FlxTimer)
			{
				var white:FlxSprite = new FlxSprite(0, 0).makeGraphic(3000, 3000);
				add(white);
				FlxTween.tween(tbc, { alpha: 1 }, 2);
			});
		}
